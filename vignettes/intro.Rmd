...
We have also to specify the library of algorithms that we want to use to predict the event of interest and to be used to form the ensemble. We could see all the algorithms that the user could potentially include in the analysis through __ensBagg::ens.all.algorithms()__. Let 's use all of them and we denote it as __ens.library__.
...
````{r, warning=FALSE}
pred <- ensBagg::ensBagg(train.data = train,test.data = test,xnam = xnam,tao = 26.5,weighting = "CoxPH",folds = 5,ens.library = ens.library )
````
...
The optimal coefficients used to get the ensemble IPCW Bagging:

````{r}
pred$optimal_coefficients
````
We can note that the algorithms with the largest predictive performance weigh in more in the ensemble. We check if convergence has been reached in the optimization problem of finding the optimal coefficients (0 denotes convergence, 1 otherwise) and the penalization term used:
````{r}
pred$convergence
...
Let s compare the ROC curve for the best and worst single algorithm and the ensemble.
The ensemble ROC curve is


````{r}
ensBagg::plot_roc(time=test$ttilde,delta = test$delta,marker =pred$prediction_ensBagg[,"Ensemble"],wts=pred$wts_test,tao=26.5,method = "ipcw")
````
...
````{r}
ensBagg::plot_roc(time=test$ttilde,delta = test$delta,marker =pred$prediction_ensBagg[,"ens.gam.4"],wts=pred$wts_test,tao=26.5,method = "ipcw")
````
...
````{r}
ensBagg::plot_roc(time=test$ttilde,delta = test$delta,marker =pred$prediction_ensBagg[,"ens.knn"],wts=pred$wts_test,tao=26.5,method = "ipcw")
````
...
pred.discard <- ensBagg::prediction_discard(train.data = train,test.data = test,xnam = xnam,tao = 26.5,ens.library=ens.library)
head(pred.discard$prediction_discard)
pred.discard$auc_discard
ensBagg::plot_roc(time=test$ttilde,delta = test$delta,marker =pred.discard$prediction_discard[,"ens.gam.4"],tao=26.5,method = "discard")
