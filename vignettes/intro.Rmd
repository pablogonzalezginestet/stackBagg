...
````{r, warning=FALSE}
pred <- ensBagg::ensBagg(train.data = train,test.data = test,xnam = xnam,tao = 26.5,weighting = "CoxPH",folds = 5,ens.library = )
````
...
````{r}
ensBagg::plot_roc(time=test$ttilde,delta = test$delta,marker =pred$prediction_ensBagg[,"Ensemble"],wts=pred$wts_test,tao=26.5)
````
...
````{r}
ensBagg::plot_roc(time=test$ttilde,delta = test$delta,marker =pred$prediction_ensBagg[,"GAM.4"],wts=pred$wts_test,tao=26.5)
````
...
````{r}
ensBagg::plot_roc(time=test$ttilde,delta = test$delta,marker =pred$prediction_ensBagg[,"k-NN"],wts=pred$wts_test,tao=26.5)
````
...
pred.discard <- ensBagg::prediction_discard(train.data = train,test.data = test,xnam = xnam,tao = 26.5)
head(pred.discard$prediction_discard)
pred.discard$auc_discard
ensBagg::plot_roc(time=test$ttilde,delta = test$delta,marker =pred.discard$prediction_discard[,"GAM.4"],tao=26.5,method = "discard")
